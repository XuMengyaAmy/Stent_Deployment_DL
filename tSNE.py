...
def get_data(Input_path, file_path): #Input_path为你自己原始数据存储路径，我的路径就是上面的'./Images'
    Image_names=os.listdir(Input_path) #获取目录下所有图片名称列表
    data=np.zeros((len(Image_names),31*34*34)) #初始化一个np.array数组用于存数据
...
def main():
    ...
    data, label, n_samples, n_features = get_data(Input_path, file_path) #根据自己的路径合理更改
